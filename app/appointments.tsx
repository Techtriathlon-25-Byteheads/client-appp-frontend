...
export default function AppointmentsScreen() {
  ...
  const pickerValue = pickingStart ? (startDate ?? new Date()) : (endDate ?? startDate ?? new Date());
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView style={styles.sheet} contentContainerStyle={styles.sheetContent} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.ongoingCard}>
          <View style={styles.ongoingHeader}>
            <View style={styles.titleRow}>
              <View style={styles.iconBadge}>
                <FontAwesome name="calendar" size={18} color={TEAL} />
              </View>
              <View>
                <Text style={styles.cardTitle}>{ongoing.title}</Text>
                <Text style={styles.cardSubtitle}>{ongoing.place}</Text>
              </View>
            </View>
            <TouchableOpacity style={styles.callBtn} accessibilityLabel="Call hospital">
          ...
          <View style={styles.chipsRow}>
            <View style={styles.chip}>
              ...
              <Text style={styles.chipText}>{ongoing.dateLabel}</Text>
            </View>
            <View style={styles.chip}>
              <FontAwesome name="clock-o" size={12} color="#FFFFFF" />
              <Text style={styles.chipText}>{ongoing.timeLabel}</Text>
            </View>
          </View>
        </View>
        ...
        {/* Previous Section */}
        <Text style={[styles.sectionLabel, { marginTop: 12 }]}>Previous Appointments</Text>
        <View style={{ gap: 12 }}>
              ...
              <View style={styles.prevHeader}>
                <View style={styles.titleRow}>
                  <View style={[styles.iconBadge, { backgroundColor: '#ECEFF1' }]}>
                    <FontAwesome name="calendar" size={18} color={TEXT_SECONDARY} />
                  </View>
                  <View>
...
const styles = StyleSheet.create({
  ...
  headerBg: { height: 160, backgroundColor: BRAND_GREEN, paddingHorizontal: 20, justifyContent: 'center' },
  ...
  ongoingCard: { backgroundColor: TEAL, borderRadius: 14, padding: 14 },
  ongoingHeader: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' },
  titleRow: { flexDirection: 'row', alignItems: 'center' },
  iconBadge: { backgroundColor: '#FFFFFF', opacity: 0.9, padding: 8, borderRadius: 10, marginRight: 10 },
  cardTitle: { color: '#FFFFFF', fontSize: 16, fontWeight: '700' },
  cardSubtitle: { color: 'rgba(255,255,255,0.85)', fontSize: 12, marginTop: 2 },
  callBtn: { backgroundColor: 'rgba(255,255,255,0.25)', borderRadius: 16, padding: 8 },
  ...
  chip: { flexDirection: 'row', alignItems: 'center', gap: 8, backgroundColor: CHIP_BG, paddingVertical: 8, paddingHorizontal: 12, borderRadius: 10 },
  chipText: { color: '#FFFFFF', fontSize: 12 },

  dateLabel: { marginTop: 16, fontSize: 14, color: TEXT_SECONDARY },
  dateRangeBtn: { marginTop: 8, height: 44, borderWidth: 1, borderColor: '#E3E5E7', borderRadius: 10, paddingHorizontal: 14, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' },
  dateRangeText: { color: TEXT_PRIMARY, fontSize: 14, fontWeight: '600' },

  prevCard: { backgroundColor: CARD_BG, borderRadius: 14, padding: 14 },
  prevHeader: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' },
  prevTitle: { color: TEXT_PRIMARY, fontSize: 15, fontWeight: '700' },
  prevSubtitle: { color: TEXT_SECONDARY, fontSize: 12, marginTop: 2 },
  prevChipsRow: { flexDirection: 'row', justifyContent: 'space-between', marginTop: 10 },
  prevChip: { flex: 1, marginRight: 8, flexDirection: 'row', alignItems: 'center', gap: 8, backgroundColor: '#E6E9ED', paddingVertical: 10, paddingHorizontal: 12, borderRadius: 10 },
