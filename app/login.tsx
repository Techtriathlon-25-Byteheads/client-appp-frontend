...
const SHEET_H_LOGIN = SCREEN_H * 0.75;
const SHEET_H_SIGNUP = SCREEN_H * 0.8;
...
const I18N: Record<Lang, Record<string, string>> = {
  ...
  ta: {
    ...
    getStartedSubtitle:
      "தேசிய சுகாதார மையத்தைப் பயன்படுத்த பதிவு செய்யவோ உள்நுழையவோ செய்யுங்கள்",
...
export default function LoginScreen() {
  ...
  useEffect(() => {
    // Function to log all storage items
  ...
  const dobOK = (d: Date | null) =>
    !!d && d.getTime() > MIN_DOB.getTime() && d.getTime() <= TODAY.getTime();
  ...
  const validateLogin = async (): Promise<boolean> => {
    ...
    if (!next.phone && !next.nic) {
      try {
        ...
        const response = await api.login(nic, formattedPhone);
        ...
        setErrors((prev) => ({ ...prev, apiError: true }));
        return false;
      } finally {
        setIsLoading(false);
      }
  ...
  const setLangAndStore = async (value: string) => {
    try {
      setLang(value as Lang);
      await AsyncStorage.setItem("appLanguage", value); // store in local storage
    } catch (err) {
          ...
          >
            ...
            {mode === "login" && (
              <>
                ...
                <View
                  style={[styles.inputWrapper, errors.nic && styles.inputError]}
                >
                  <TextInput
                    style={styles.input}
                    value={nic}
                ...
                >
                  ...
                  >
                    <Text
                      style={[
                        styles.input,
                        { color: dob ? "#111827" : "#9AA3AE", flex: 1 },
                      ]}
                    >
          ...
          >
            ...
            <View style={[styles.segment, styles.segmentLang]}>
              <TouchableOpacity
                style={[
                  styles.segmentItem,
                  lang === "si" && styles.segmentItemActive,
                ]}
                onPress={() => setLangAndStore("si")}
                activeOpacity={0.85}
              >
                <Text
                  style={[
                    styles.segmentText,
                    lang === "si" && styles.segmentTextActive,
                  ]}
                >
                  {t("lang_si")}
                </Text>
              ...
              <TouchableOpacity
                style={[
                  styles.segmentItem,
                  lang === "ta" && styles.segmentItemActive,
                ]}
                onPress={() => setLangAndStore("ta")}
                activeOpacity={0.85}
              >
                <Text
                  style={[
                    styles.segmentText,
                    lang === "ta" && styles.segmentTextActive,
                  ]}
                >
                  {t("lang_ta")}
                </Text>
              ...
              <TouchableOpacity
                style={[
                  styles.segmentItem,
                  lang === "en" && styles.segmentItemActive,
                ]}
                onPress={() => setLangAndStore("en")}
                activeOpacity={0.85}
              >
                <Text
                  style={[
...
function Checkbox({ checked }: { checked: boolean }) {
  return (
    <View style={[styles.checkboxBase, checked && styles.checkboxBaseChecked]}>
      {checked ? <Feather name="check" size={14} color="#24786D" /> : null}
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  helpLink: { color: "#24786D", fontWeight: "700" },
